...
var maxDashDuration: float = 0.3
var maxDashCD: float = 5.0
...
func _physics_process(delta: float) -> void:
	...
	if player_node != null:
		# Dashes towards player when not on cooldown else goes at chasing speed
		if dashing:
			...
			dashCD -= delta
			if dashCD <= 0.0 and canDash:
				dashDirection = (player_node.position - position).normalized()
		...
		if (
			...
			if idle_animation_suffix not in _animated_sprite_2d.animation:
